...
- [x] Fresh VPS (Ubuntu 22.04/24.04, deploy user with sudo, no Caddy/Node/PM2)
- [x] Run `./setup-vps.sh` with static domain config
- [x] VPS has Caddy installed and running
- [x] `/etc/caddy/domains.json` created with domain entry
- [x] `/var/www/example.com/` exists and owned by deploy
- [x] Caddyfile generated and Caddy reloaded
- [x] Run `./deploy.sh` (or `./deploy.sh --skip-build` if build already done)
- [x] Static files transferred to `/var/www/example.com/`
- [x] `https://example.com` returns 200 and serves site
- [x] SSL certificate obtained automatically by Caddy
