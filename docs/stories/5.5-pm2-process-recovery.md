...
- [x] Dynamic domain running with PM2
- [x] `pm2 startup` has been configured
- [x] Reboot VPS: `sudo reboot`
- [x] After reboot, check `systemctl status pm2-deploy` (or similar)
- [x] PM2 service starts automatically
- [x] Check `pm2 list` shows app in "online" status
- [x] If app not started, `pm2 resurrect` may be needed (document behavior)
