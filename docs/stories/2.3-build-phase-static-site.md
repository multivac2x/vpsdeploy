...
- [x] `BUILD_CMD` is executed (default: `npm run build`)
- [x] After build, `BUILD_OUTPUT` directory exists (default: `out/`)
- [x] Script counts files in output directory and logs count
- [x] If `BUILD_OUTPUT` missing after build, error and exit code 1
- [x] If `--skip-build` used, build is skipped and `BUILD_OUTPUT` must already exist
