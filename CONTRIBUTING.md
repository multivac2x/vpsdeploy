# Contributing to vps-deploy

Thank you for your interest in contributing to vps-deploy! This document provides guidelines and information for contributors.

## Development Workflow

### Story-Based Development

All work is organized into stories documented in [`docs/stories/`](docs/stories/). Each story has:
...
1. Select a story from [`docs/EPICS.md`](docs/EPICS.md)
...
- **Exported variables**: Configuration variables are exported in [`lib/config.sh`](lib/config.sh) to make them visible to sourced modules.
...
- Optional variables have sensible defaults in [`lib/config.sh`](lib/config.sh)
...
Use the logging functions from [`lib/logging.sh`](lib/logging.sh):
...
- Keep [`README.md`](README.md) in sync with changes
- Update [`docs/manual-setup-guide.md`](docs/manual-setup-guide.md) if user-facing steps change
- Update [`docs/PRD.md`](docs/PRD.md) if product behavior changes
- Add new configuration variables to [`.env.deploy.example`](.env.deploy.example)

## Questions?

Open an issue or refer to the existing documentation in the `docs/` directory.
